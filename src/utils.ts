...
const Utils = {
  ...
  getNumericWord(number: number, words: string[]) {
    const cases = [2, 0, 1, 1, 1, 2];
    return words[(number % 100 > 4 && number % 100 < 20) ? 2 : cases[(number % 10 < 5) ? number % 10 : 5]];
  },

  returnFullStringTime(time: number) {
    const { days, hours } = this.getStringTime(time);
    return `${days} ${this.getNumericWord(days, ['день', 'дня', 'дней'])} ${hours !== 0 ? `${hours} ${this.getNumericWord(hours, ['час', 'часа', 'часов'])}` : ""}`;
  },

  returnEpisodeString(anime: any) {
  ...
  returnTimeString(time: number, showYear = false) {
  ...
  returnFormatedTime(time: number) {
  ...
  async fallback(callback: (success: boolean) => void, count: number) {
    let success = false;
